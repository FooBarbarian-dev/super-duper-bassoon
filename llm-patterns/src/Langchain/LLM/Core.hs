...
-- | OpenAI instance with real API calls (with retry logic)
  ...
  chat model messages _params = do
    ...
    if T.null apiKey
      then do
        ...
        putStrLn "🔑 OpenAI API: API key found, making request with retry logic..."
        -- Use retry logic with exponential backoff
        result <- withRetry defaultRetryConfig $ makeOpenAIRequest apiKey (openaiModelName model) messages
        case result of
          Left err -> do
            putStrLn $ "❌ OpenAI API failed after retries: " ++ err
            pure $ Left err
          Right txt -> do
            putStrLn $ "✅ OpenAI API success: " ++ show (T.length txt) ++ " chars"
            pure $ Right txt

-- | Claude instance with real API calls (with retry logic)
  ...
  chat model messages _params = do
    ...
    if T.null apiKey
      then do
        ...
        putStrLn "🔑 Claude API: API key found, making request with retry logic..."
        -- Use retry logic with exponential backoff
        result <- withRetry defaultRetryConfig $ makeClaudeRequest apiKey (claudeModelName model) messages
        case result of
          Left err -> do
            putStrLn $ "❌ Claude API failed after retries: " ++ err
            pure $ Left err
          Right txt -> do
            putStrLn $ "✅ Claude API success: " ++ show (T.length txt) ++ " chars"
            pure $ Right txt

-- | Make OpenAI API request with 60 second timeout and error handling
makeOpenAIRequest :: Text -> Text -> [Message] -> IO (Either String Text)
makeOpenAIRequest apiKey modelName messages = do
  result <- try makeRequest
  case result of
    Left (e :: SomeException) -> do
      let errMsg = "OpenAI API request failed: " ++ show e
      putStrLn $ "❌ " ++ errMsg
      pure $ Left errMsg
    Right r -> pure r
  where
...
-- | Make Claude API request with 60 second timeout and error handling
makeClaudeRequest :: Text -> Text -> [Message] -> IO (Either String Text)
makeClaudeRequest apiKey modelName messages = do
  result <- try makeRequest
  case result of
    Left (e :: SomeException) -> do
      let errMsg = "Claude API request failed: " ++ show e
      putStrLn $ "❌ " ++ errMsg
      pure $ Left errMsg
    Right r -> pure r
  where
